...
{                                                                                                                   
	...
	# values over 15V are considered bogus, too
	if [ $val -gt 15000 ]; then
		volts_ok=0
	fi
}                                                                                                                   
...
${curl} -q -k -H "Content-Type: application/json" -H "Authorization: Bearer ${access_token}" -X POST -d "[{\"t\": $(date +%s)000, \"pid\": 0, \"n\": \"avatar\", \"v\": \"https://uavpal.com/img/${tn_filename}?$(date +%s)\"}]" "https://api.glympse.com/v2/tickets/$ticket/append_data"

ztVersion=$(/data/ftp/uavpal/bin/zerotier-one -v)

ulogger -s -t uavpal_glympse "... Glympse API: reading out drone's GPS coordinates, battery voltage, connection type and signal strength approx. every 5 seconds to update Glympse via WebAPI"
...
do
        crc_ok=0
	volts_ok=0
	# Try to get valid i2c data 5 times
        for i in $(seq 1 5); do
	...
	if [ -f /data/ftp/internal_000/*/academy/*.pud.temp ]; then
		altitude_rel=$(${dc} -e "$altitude_abs $(cat /tmp/alt_before_takeoff) - p")
	else
	...
	latency="n/a"
	if [ -f /tmp/sc2ping ]; then
		if [ `cat /tmp/sc2ping | wc -l` -eq '1' ]; then
			latency=$(${dc} -e "$(cat /tmp/sc2ping) 2 / p")ms
		fi
	fi
	...
	if [ `echo $ip_sc2 | awk -F. '{print $1"."$2"."$3}'` == "192.168.42" ]; then
		...
		if [ ! -f "/tmp/hilink_router_ip" ]; then
			...
			signalPercentage=$(echo "$signalBars 20 * p" | ${dc})%
			signal="$mode/$signalPercentage"
		fi
	fi
	...
	${curl} -q -k -H "Content-Type: application/json" -H "Authorization: Bearer ${access_token}" -X POST -d "[[$(date +%s)000,$(gpsDecimal $lat $latdir),$(gpsDecimal $long $longdir),$speed,$heading]]" "https://api.glympse.com/v2/tickets/$ticket/append_location" &
	...
	while ps |grep curl |grep -v grep >/dev/null; do sleep 1; done
